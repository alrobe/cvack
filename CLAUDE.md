...
- Restore reuses the existing `importJSON()` (parses, merges with `empty`, persists to `localStorage`, re-renders) — it's the same code path as the "Importar Json" file button, just fed by Drive content instead of a local file. It's gated behind a `confirm()`, matching the "Nuevo CV" button's pattern of warning before discarding unsaved local edits.
