...
- The `drive.appdata` scope is classified as **non-sensitive** by Google (same tier as `drive.file`), so publishing the OAuth app from "Testing" to "In production" (which lifts the ~100-test-user cap) does not require sensitive-scope review (no demo video or scope justification). Optional brand verification (needed to show the CVack name/logo on the consent screen) requires the homepage and `privacy.html` to be public, the app name to match, and ownership of `alrobe.github.io` verified in Google Search Console. Adding any sensitive/restricted scope later would trigger full verification.
