...
- `privacy.html`, styled by `css/legal.css` — standalone privacy policy page, linked from the home page footer and required by Google's OAuth consent screen for the `drive.appdata` scope (see "Google Drive backup" below).
...
- The `drive.appdata` scope is classified as sensitive by Google (same tier as `drive.file`): since CVack is public, the OAuth consent screen eventually needs Google verification (privacy policy required — see `privacy.html`) once usage grows beyond the ~100-test-user "Testing" mode cap — until verified, public users see an "unverified app" warning screen.
