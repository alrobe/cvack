...
Each script defines its own `STORAGE_KEY` (`cvack-general` / `cvack-dev`, used by `common.js`'s `save()`/`importJSON()` and by the page's own initial `data` load), so the two CV types no longer share saved data. **Note**: the Google Drive backup file id/token cache (`orange-cv-drive-file-id`, `orange-cv-drive-token`) is still shared between both editors — see "Google Drive backup" below.
